...
- Railway must be configured with `SENDGRID_API_KEY`, `SMTP_FROM`, and `ALCHEMY_API_KEY` in its environment settings for OTP email delivery and blockchain provider lookups.
- `SMTP_FROM` must be a verified sender address in SendGrid so transactional OTP emails can be delivered.
