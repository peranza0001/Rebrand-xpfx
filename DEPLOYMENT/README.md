...
- Railway must be configured with `SENDGRID_API_KEY`, `EMAIL_FROM`, and `ALCHEMY_API_KEY` in its environment settings for OTP email delivery and blockchain provider lookups.
- `EMAIL_FROM` must be a verified sender address in SendGrid so transactional OTP emails can be delivered.
